...
void OutdatedState::draw(C3D_RenderTarget* top, C3D_RenderTarget* bottom) {
        ...
        for (float x = -gridSize; x < 400 + gridSize; x += gridSize) {
            for (float y = -gridSize; y < 240 + gridSize; y += gridSize) {
                if ((int(x / gridSize) + int(y / gridSize)) % 2 == 0) {
                    C2D_DrawRectSolid(x + gx + get3DOffset(-10.0f), y + gy, 0.01f, gridSize, gridSize, gridCol);
                }
            }
        }
    } else {
    ...
    C2D_DrawText(&titleText, C2D_WithColor, 200.0f - tw/2.0f + get3DOffset(10.0f), 20.0f, 0.5f, 0.6f, 0.6f, (comparison < 0) ? C2D_Color32(255, 0, 0, 255) : C2D_Color32(100, 255, 100, 255));
    ...
    for (int i = 0; i < bodyTextLines && i < 12; i++) {
        ...
        C2D_DrawText(&bodyText[i], C2D_WithColor, 200.0f - tw/2.0f + get3DOffset(10.0f), startY, 0.5f, 0.4f, 0.4f, col);
        startY += 18.0f;
    }
